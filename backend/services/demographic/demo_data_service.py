...
class DemographicDataService:
    ...
    def _import_dependencies(self) -> dict[str, Any]:
        ...
        try:
            from services.demographic.demo_data_loader import load_demographic_csv
    ...
    def _resolve_latest_non_empty_label(self, column: str, upto_year: int) -> str | None:
        ...
        if column not in self._canonical_df.columns:
            return None

        subset = self._canonical_df[
            (self._canonical_df["report_year"] <= upto_year)
            & self._canonical_df[column].notna()
    ...
    def get_kpis(self, year: int | None = None) -> dict[str, Any]:
        ...
        dominant_age_cohort = self._resolve_latest_non_empty_label(
            "dominant_age_cohort", target_year
        )
        dominant_purpose = self._resolve_latest_non_empty_label(
            "dominant_purpose", target_year
        )
        ...
        return {
            ...
            "dominant_age_cohort": dominant_age_cohort,
