...
function SubscriptionModal({ isOpen, onClose, onUpgrade, onContinueFree }) {
  ...
  return (
    <div
      className="fixed inset-0 z-[1000] overflow-y-auto bg-black/70 p-4 backdrop-blur-[2px]"
      onMouseDown={(event) => {
        if (event.target === event.currentTarget) onClose();
      }}
      aria-hidden={false}
    >
      <div className="mx-auto flex min-h-[100svh] w-full items-start justify-center py-4">
        <div
          ref={dialogRef}
          role="dialog"
          aria-modal="true"
          ...
          className="w-full max-w-5xl max-h-[calc(100svh-2rem)] overflow-y-auto rounded-2xl border border-gray-700/70 bg-[#0f1115] text-white shadow-2xl"
        >
              ...
              {plans.map((plan) => {
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={plan.id === 'pro' ? onUpgrade : continueFreeHandler}
                      className={[
